...
** auto-install.el
*** インストール
#+BEGIN_SRC sh :tangle install-auto-install.sh :var eliep_dir=tnoda-elisp-dir
...
*** auto-install を =~/.emacs.d/auto-install= で使う設定
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'load-path (expand-file-name "popwin-el" tnoda/src-dir))
  (require 'popwin)
...
* Org Mode                                                              :git:
** インストール
+ git://orgmode.org/org-mode.git をサブモジュールにする．
+ git submodule init/update  後に次のスクリプトを実行してコンパイル・ info インストール．
  #+BEGIN_SRC sh :tangle setup-org.sh :var src_dir=tnoda-src-dir prefix=tnoda-dotfiles-dir emacs=emacs-app
    cd $src_dir/org
...
** emacs-w3m                                                           :cvs:
*** インストール
#+BEGIN_SRC sh :tangle install-emacs-w3m.sh :var src_dir=tnoda-src-dir prefx=tnoda-dotfiles-dir elisp_dir=tnoda-elisp-dir emacs=emacs-app
  cd $src_dir
...
+ =ruby-mode.el= などは GitHub から最新ものを入手したい．
  #+BEGIN_SRC emacs-lisp :tangle install.el
    (auto-install-from-url "https://raw.github.com/ruby/ruby/trunk/misc/ruby-mode.el")
    (auto-install-from-url "https://raw.github.com/ruby/ruby/trunk/misc/ruby-electric.el")
    (auto-install-from-url "https://raw.github.com/ruby/ruby/trunk/misc/inf-ruby.el")
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'load-path (expand-file-name "~/.rbenv/shims"))
#+END_SRC
...
** Markdown                                                             :git:
*** git://jblevins.org/git/markdown-mode.git を submodule にして使う．
#+BEGIN_SRC emacs-lisp :tangle yes
  (add-to-list 'load-path (expand-file-name "markdown-mode" tnoda/src-dir))
