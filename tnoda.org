...
*** clojure-mode
**** =clojure-mode= の =<f5>= で =nrepl-ritz-jack-in=.          :key:command:
#+BEGIN_SRC emacs-lisp :tangle yes
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (el-get 'sync '(ruby-mode ruby-electric inf-ruby ruby-compilation yari flymake-easy flymake-ruby))
  (require 'ruby-mode)
  (require 'inf-ruby)
...
#+BEGIN_SRC emacs-lisp :tangle yes
  ...
  (add-hook 'ruby-mode-hook 'tnoda/subword-mode-enable)
  (add-hook 'ruby-mode-hook 'tnoda/enable-show-trailing-whitespaces)
#+END_SRC
...
#+BEGIN_SRC sh :tangle install-org.sh :var src_dir=tnoda-src-dir prefix=tnoda-dotfiles-dir info_dir=tnoda-info-dir emacs=emacs-app
  cd $src_dir
  curl -O http://orgmode.org/org-7.9.3f.tar.gz
  tar zxf org-7.9.3f.tar.gz
  rm -f org
  ln -s org-7.9.3f org
  cd org
