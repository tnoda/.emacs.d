...
#+TAGS:  key(k) command(c) autoinstall(a) marmalade(m) git(g) svn(s) rubikitch(r)
...
*** =C-c w= で Capture Away 開始．                                      :key:
#+BEGIN_SRC emacs-lisp :tangle yes
  (defun tnoda/org-capture-away ()
    (interactive)
    (org-capture nil "a"))
  (global-set-key (kbd "C-c w") 'tnoda/org-capture-away)
#+END_SRC
...
*** Spork サーバを =*spork*= バッファに起動するコマンド =tnoda/spork-start=. :command:

+ =tnoda/spork-stop= で停止する．
+ エイリアスとして =spork-start=, =spork-stop= を用意．

#+BEGIN_SRC emacs-lisp :tangle yes
  (defun tnoda/spork-start ()
    "Start the Spork server."
    (interactive)
    (setq tnoda/spork-command (format "cd %s && spork testunit" (rinari-root)))
  ...
  (defalias 'spork-start 'tnoda/spork-start)
  (defalias 'spork-stop 'tnoda/spork-stop)
#+END_SRC
...
+ =ghc*.el= のコンパイルとインストール
  #+BEGIN_SRC sh :tangle install-ghc-mod.sh :var emacs=emacs-app elisp_dir=tnoda-elisp-dir
    ...
    cabal install hlint
    make -C ~/Library/Haskell/ghc-7.0.3/lib/ghc-mod-1.10.6/share EMACS=$emacs
    mkdir -p $elisp_dir/ghc-mod
    cp ~/Library/Haskell/ghc-7.0.3/lib/ghc-mod-1.10.6/share/*.elc $elisp_dir/ghc-mod
  #+END_SRC
