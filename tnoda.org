...
#+BEGIN_SRC sh :tangle install-org.sh :var src_dir=tnoda-src-dir prefix=tnoda-dotfiles-dir info_dir=tnoda-info-dir emacs=emacs-app
  cd $src_dir
  curl -O http://orgmode.org/org-7.9.2.tar.gz
  tar zxf org-7.9.2.tar.gz
  ln -s org-7.9.2 org
  cd org
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq org-todo-keywords
        '((sequence "TODO(t)" "STARTED" "PENDING" "WAITING(w@/!)" "APPT(a)" "|"
                    "DONE(d!)" "CANCELLED(c@)" "DEFERRED(f@)" "DELEGATED(g@/!)")))
#+END_SRC

*** クロックを開始したタスクはステータスを =STARTED= に変更する．
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq org-clock-in-switch-to-state "STARTED")
#+END_SRC

*** クロックを停止したタスクはステータスを =PENDING= に変更する．
#+BEGIN_SRC emacs-lisp :tangle yes
  (setq org-clock-out-switch-to-state "PENDING")
#+END_SRC
