...
#+BEGIN_SRC emacs-lisp :tangle yes
  (unless (require 'sequential-command nil t)
    (let ((url "https://gist.githubusercontent.com/tnoda/49797ef440b7a2166986/raw/sequential-command.el"))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (unless (require 'sense-region nil t)
    (let ((url "https://gist.githubusercontent.com/tnoda/1776988/raw/f1421879e6f9c96a1a0b9708bc8da0378f30e22b/sense-region.el"))
      (-if-let (buf (url-retrieve-synchronously url))
          (progn
            (switch-to-buffer buf)
            (package-install-from-buffer)))))
...
#+BEGIN_SRC emacs-lisp :tangle yes
  (unless (require 'mell nil t)
    (let ((url "https://gist.githubusercontent.com/tnoda/dedb18a47780e9a50983/raw/eb99fe55f813f14b75009923301033cb17d601c2/mell.el"))
      (-if-let (buf (url-retrieve-synchronously url))
          (progn
            (switch-to-buffer buf)
            (package-install-from-buffer)))))

  (unless (require 'text-adjust nil t)
    (let ((url "https://gist.githubusercontent.com/tnoda/dedb18a47780e9a50983/raw/eb99fe55f813f14b75009923301033cb17d601c2/text-adjust.el"))
      (-if-let (buf (url-retrieve-synchronously url))
          (progn
            (switch-to-buffer buf)
            (package-install-from-buffer)))))
